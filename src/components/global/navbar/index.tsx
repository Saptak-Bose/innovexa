    ...
    <header className="fixed top-0 right-0 left-0 p-4 bg-black/40 backdrop-blur-lg z-[100] flex items-center border-b border-border justify-between">
      ...
      <nav className="absolute left-[50%] top-[50%] transform translate-x-[-50%] translate-y-[-50%] hidden md:block">
        <ul className="flex items-center gap-4 list-none">
          <li className="hover:underline hover:underline-offset-2 hover:transition-all hover:transform hover:ease-in-out hover:duration-300 hover:font-bold hover:text-primary text-secondary-foreground font-semibold">
      ...
      <aside className="flex items-center gap-4">
        <Link href={`/${user?.id}`} className="focus:outline-none">
